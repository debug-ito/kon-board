...
import qualified KonBoard.Recipe        as Recipe
...
loadYamls ms = basicLogging $ do
  ...
  mapM_ (Recipe.loadYamlFile rs) recipeFiles
...
specForStore storeName = beforeWith loadYamls $ describe storeName $ do
  describe "searchMealPlans" $ do
    specify "empty result" $ \s -> do
      got <- basicLogging $ searchMealPlans s (gday 2020 3 1) (gday 2020 3 20)
      got `shouldBe` []
    specify "exclusive range" $ \s -> do
      ...
      got <- basicLogging $ searchMealPlans s (gday 2020 4 1) (gday 2020 4 10)
    ...
    specify "lunch and dinner" $ \s -> do
      ...
      got <- basicLogging $ searchMealPlans s (gday 2020 4 2) (gday 2020 4 11)
      map planWithoutID got `shouldBe` expected
    specify "sorted by gday and phase" $ \s -> do
      ...
      got <- basicLogging $ searchMealPlans s (gday 2020 4 23) (gday 2020 5 16)
      map planWithoutID got `shouldBe` expected
    specify "MealPlan with multiple recipes" $ \s -> do
      let expected =
            [ (gday 2019 11 21, Dinner, ["recipe 1", "recipe 2"], [])
            ]
      got <- basicLogging $ searchMealPlans s (gday 2019 11 1) (gday 2019 11 30)
      map planWithoutID got `shouldBe` expected
    specify "MealPlan in the example" $ \s -> do
      ...
      got <- basicLogging $ searchMealPlans s (gday 2019 3 1) (gday 2019 5 1)
      map planWithoutID got `shouldBe` expected
    specify "MealPlan with MealOther phases" $ \s -> do
      ...
      got <- basicLogging $ searchMealPlans s (gday 2020 7 10) (gday 2020 7 11)
      map planWithoutID got `shouldBe` expected
    specify "MealPlan with notes and possible no recipes" $ \s -> do
      ...
      got <- basicLogging $ searchMealPlans s (gday 2020 7 15) (gday 2020 7 16)
      map planWithoutID got `shouldBe` expected
